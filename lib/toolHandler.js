...
class DownloadExtractInstall {
    ...
    async isAlreadyInstalled(toolName) {
        const cachePath = await (0, tool_cache_1.find)(toolName, '*');
        const systemPath = await (0, io_1.which)(toolName);
        if (cachePath)
            return cachePath;
        if (systemPath) {
    ...
    async _getVersion(installedBinary) {
        ...
        const installedVersion = (0, util_1._filterVersion)(versionCommandOutput);
    ...
    async _getCommandOutput(commandStr, args) {
        ...
        if (IS_WINDOWS) {
            ...
            args.unshift('/c');
            commandStr = 'cmd';
            await (0, exec_1.exec)(commandStr, args, options);
            return await (0, util_1._readFile)(logFile, {});
        }
        else {
            await (0, exec_1.exec)(commandStr, args, options);
            if (stdOut === '')
    ...
    async downloadFile() {
        const filePath = await (0, tool_cache_1.downloadTool)(this.downloadUrl);
        const destPath = `${filePath}${this.fileType}`;
        await (0, io_2.mv)(filePath, destPath);
        return destPath;
    }
    async extractFile(filePath) {
        ...
        if (process.platform === 'linux') { // Workaround
            await (0, exec_1.exec)(`unzip ${filePath}`, ['-d', extractDir]);
            return extractDir;
        }
        return await (0, tool_cache_1.extractZip)(filePath, extractDir);
    }
    async installPackage(installCommand, installArgs) {
        return await (0, exec_1.exec)(installCommand, installArgs);
    }
    async cacheTool(installedBinary) {
        const installedVersion = await this._getVersion(installedBinary);
        const cachedPath = await (0, tool_cache_1.cacheDir)(path.parse(installedBinary).dir, 'aws', installedVersion);
        return cachedPath;
    }
}
